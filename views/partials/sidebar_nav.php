...
<nav class="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto px-3 py-4">
    ...
    <?php else: ?>
        ...
        <div class="space-y-1">
            ...
            >
                <svg class="h-5 w-5 flex-shrink-0 text-gray-400 transition-colors group-hover:text-gray-500" :class="activeView === 'reports' ? 'text-white group-hover:text-white' : ''" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z"></path>
                </svg>
                <span><?= htmlspecialchars(__('nav_reports'), ENT_QUOTES, 'UTF-8') ?></span>
            </button>
            <button
                type="button"
            ...
            >
                <svg class="h-5 w-5 flex-shrink-0 text-gray-400 transition-colors group-hover:text-gray-500" :class="activeView === 'documents' ? 'text-white group-hover:text-white' : ''" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"></path>
                </svg>
                <span><?= htmlspecialchars(__('nav_documents'), ENT_QUOTES, 'UTF-8') ?></span>
            </button>
            <button
                type="button"
            ...
            >
                <svg class="h-5 w-5 flex-shrink-0 text-gray-400 transition-colors group-hover:text-gray-500" :class="activeView === 'announcements' ? 'text-white group-hover:text-white' : ''" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10.34 15.84c-.688-.06-1.386-.09-2.09-.09H7.5a4.5 4.5 0 110-9h.75c.704 0 1.402-.03 2.09-.09m0 9.18c.253.962.584 1.892.985 2.783.247.55.06 1.21-.463 1.511l-.657.38c-.551.318-1.26.117-1.527-.461a20.932 20.932 0 01-1.733-5.288m12.733 0c-.62-.2-1.252-.368-1.895-.498m1.895.498c.387 1.382.738 2.793 1.046 4.222.172.8-.37 1.548-1.182 1.548h-1.91a.75.75 0 01-.688-.45 19.07 19.07 0 00-1.733-5.288m0 0A19.116 19.116 0 0012.75 6.75"></path>
                </svg>
                <span><?= htmlspecialchars(__('nav_announcements'), ENT_QUOTES, 'UTF-8') ?></span>
            </button>
            <?php endif; ?>
        </div>
        <?php endif; ?>
        ...
        <div class="space-y-1">
            ...
            <div x-show="assetManagementOpen" x-cloak class="ml-4 space-y-1 border-l border-gray-200 pl-3">
                ...
                <a
                    ...
                    :class="activeView === 'assets' && Number(activeAssetTypeId) === <?= (int) ($assetType['id'] ?? 0) ?> ? 'panel-nav-active font-semibold text-white shadow-sm' : ''"
                >
        ...
        <a
            ...
            @click.prevent="openSwitchPorts()"
            class="group flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900"
            :class="activeView === 'switch_ports' ? 'panel-nav-active font-semibold text-white shadow-sm' : ''"
        >
            <svg class="h-5 w-5 flex-shrink-0 text-gray-400 transition-colors group-hover:text-gray-500" :class="activeView === 'switch_ports' ? 'text-white group-hover:text-white' : ''" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
