...
<section x-show="activeView === 'settings'" x-cloak class="space-y-6">
    ...
    <nav class="flex flex-wrap gap-2 border-b border-zinc-200 pb-1">
        ...
        <button
            type="button"
            @click="settingsTab = 'asset_components'; fetchAssetTypes().then(() => { selectedAssetComponentTypeId = selectedAssetComponentTypeId || activeAssetTypeId || (assetTypes[0]?.id ?? null); fetchAssetTypeComponents(); })"
            class="rounded-lg px-3 py-2 text-sm font-medium transition"
            :class="settingsTab === 'asset_components' ? 'bg-zinc-900 text-white' : 'text-zinc-600 hover:bg-zinc-100'"
        >
            <?= htmlspecialchars(__('settings_tab_asset_components'), ENT_QUOTES, 'UTF-8') ?>
        </button>
        <button
            type="button"
        ...
        <article class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-soft">
            ...
            <div class="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <label class="block sm:col-span-2">
                    ...
                    <input type="text" x-model="settingsForm.smtp_config.sender_name" class="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-zinc-400" placeholder="BTECH ITMS">
                </label>
                <label class="block sm:col-span-2">
