...
<section x-show="activeView === 'system_users'" x-cloak class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-zinc-500"><?= htmlspecialchars(__('system_users_page_subtitle'), ENT_QUOTES, 'UTF-8') ?></p>
        </div>
        <button
            type="button"
    ...
    <p x-show="systemUsersLoading" x-cloak class="rounded-xl border border-zinc-200 bg-white px-4 py-6 text-sm text-zinc-500">
        <?= htmlspecialchars(__('system_users_loading'), ENT_QUOTES, 'UTF-8') ?>
    </p>
    <p x-show="systemUsersError" x-cloak class="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700" x-text="systemUsersError"></p>
    <p x-show="systemUsersSuccessMessage" x-cloak class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700" x-text="systemUsersSuccessMessage"></p>

    <div x-show="!systemUsersLoading && systemUsers.length > 0" x-cloak class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-soft">
    ...
    <p
        x-show="!systemUsersLoading && !systemUsersError && systemUsers.length === 0"
        x-cloak
        class="rounded-xl border border-dashed border-zinc-200 bg-zinc-50 px-4 py-8 text-sm text-zinc-500"
    >
        <?= htmlspecialchars(__('system_users_empty'), ENT_QUOTES, 'UTF-8') ?>
    </p>
</section>
