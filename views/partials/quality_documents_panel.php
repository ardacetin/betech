...
<section x-show="activeView === 'documents'" x-cloak class="space-y-4">
    <div class="overflow-hidden rounded-2xl border border-[var(--border,#e5e7eb)] bg-white p-6">
        ...
        <form id="quality-document-upload" @submit.prevent="submitQualityDocumentForm" class="mt-6 rounded-2xl border border-dashed border-zinc-300 bg-[var(--bg,#f7f7f5)] p-5">
            <h3 class="text-sm font-semibold text-zinc-900"><?= htmlspecialchars(__('quality_documents_modal_title'), ENT_QUOTES, 'UTF-8') ?></h3>
            <p class="mt-1 text-xs text-zinc-500"><?= htmlspecialchars(__('quality_documents_modal_subtitle'), ENT_QUOTES, 'UTF-8') ?></p>
            <div class="mt-4 grid gap-4 md:grid-cols-2">
                <label class="block md:col-span-2">
                    ...
                    <input type="text" x-model="qualityDocumentForm.title" required maxlength="255" class="w-full rounded-xl border border-zinc-300 bg-white px-3 py-2.5 text-sm outline-none ring-zinc-900/10 focus:border-zinc-400 focus:ring-4">
                </label>
                <label class="block md:col-span-2">
                ...
                <label class="inline-flex items-center gap-2 text-sm text-zinc-700 md:col-span-2">
