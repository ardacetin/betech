...
class DatabaseInitializer
{
    ...
    public function initialize(): DatabaseInitializationResult
    {
        ...
        try {
            ...
            if ($this->assetsTableExists($connection)) {
                ...
                foreach ($this->patchTicketCollaboration($connection) as $warning) {
                    $warnings[] = $warning;
                }
    ...
    /**
     * Self-heal ticket attachments, followers, internal notes, and email threading.
     *
     * @param object $connection Medoo instance
     *
     * @return list<string>
     */
    private function patchTicketCollaboration(object $connection): array
    {
        $warnings = [];

        if (!$this->tableExists($connection, 'tickets') || !$this->tableExists($connection, 'ticket_comments')) {
            return $warnings;
        }

        if (!$this->columnExists($connection, 'tickets', 'source')) {
            $connection->query(
                "ALTER TABLE tickets
                    ADD COLUMN source VARCHAR(32) NOT NULL DEFAULT 'web' AFTER created_by_user_id"
            );
            $warnings[] = 'Self-healed database: added tickets.source column.';
        }

        if (!$this->columnExists($connection, 'tickets', 'email_message_id')) {
            $connection->query(
                'ALTER TABLE tickets
            ...
            $warnings[] = 'Self-healed database: added tickets email threading columns.';
        }

        if (
            $this->columnExists($connection, 'tickets', 'email_message_id')
            && !$this->indexExists($connection, 'tickets', 'uq_tickets_email_message_id')
        ) {
            $connection->query(
                'ALTER TABLE tickets ADD UNIQUE KEY uq_tickets_email_message_id (email_message_id)'
            );
            $warnings[] = 'Self-healed database: added unique index on tickets.email_message_id.';
        }

        if (!$this->columnExists($connection, 'ticket_comments', 'is_internal')) {
            $connection->query(
            ...
            $warnings[] = 'Self-healed database: added ticket_comments internal/email columns.';
        }

        if (!$this->tableExists($connection, 'ticket_comment_attachments')) {
            $connection->query(
            ...
            $warnings[] = 'Self-healed database: created ticket_comment_attachments table.';
        }

        if (!$this->tableExists($connection, 'ticket_followers')) {
            $connection->query(
            ...
            $warnings[] = 'Self-healed database: created ticket_followers table.';
        }

        if (!$this->tableExists($connection, 'ticket_email_messages')) {
            $connection->query(
            ...
            $warnings[] = 'Self-healed database: created ticket_email_messages table.';
        }
